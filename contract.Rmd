...
advdate <- function(weekstart,
                    week,
                    topic =  NULL,
                    assignment = NULL,
                    unit="Module ") {
  tmon <- weekstart + 7*(week-1) %>%
  format(format="%m/%d")
  ttue <- weekstart + 1 + 7*(week-1) %>%
  format(format="%m/%d")
  twed <- weekstart + 2 + 7*(week-1) %>%
  format(format="%m/%d") 
  tthu <- weekstart + 3 + 7*(week-1) %>%
  format(format="%m/%d") 
  tfri <- weekstart + 4 + 7*(week-1) %>%
  format(format="%m/%d") 
  tsat <- weekstart + 5 + 7*(week-1) %>%
  format(format="%m/%d") 
  tsun <- weekstart + 6 + 7*(week-1) %>%
  format(format="%m/%d")
...
- [ ] `r advdate(dayone, 8, topic)`
  ...
  - Schedule a check-in with Mason by `r advdate(dayone, 8, assignment="f")`
